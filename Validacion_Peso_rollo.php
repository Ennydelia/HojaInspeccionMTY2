			...
			<div class= "col-lg-12 col-md-12 col-sm-12">
				<?php
            		...
            		$consulta6 = "EXEC [MTY_PROD_SSM].[dbo].[SP_INSPECCION_WO_C] @BOM_NO = '". strtoupper($_GET["bom"]) ."'";
								...
								$consulta = "select ISNULL(MOTHER_BOM, 'BALANCE') AS MOTHER_BOM FROM [MTY_PROD_SSM].[dbo].[SSM_INSPECCION_RM] WHERE MOTHER_BOM = '". strtoupper($_GET["bom"]) ."' and FINAL_CHECK is NULL or FINAL_CHECK = 0 AND MOTHER_BOM = '". strtoupper($_GET["bom"]) ."'";
							//OBTIENE LOS FORMERS BOMS DE ESE WO
								$resultado = odbc_do($conn, $consulta);
								$yavalidado = 1;
								while (odbc_fetch_row($resultado)) {
									...
									$consulta = "SELECT count(*) EDO FROM [MTY_PROD_SSM].[dbo].[SSM_INSPECCION_RM] WHERE MOTHER_BOM = '". strtoupper($_GET["bom"]) ."' and VAL_PESO_INI is NULL ";
									$resultado = odbc_do($conn, $consulta);
									while (odbc_fetch_row($resultado)) {
										if(odbc_result($resultado, 1) <> "0"){
											$consulta = "SELECT TOP 1 MOTHER_BOM, convert(varchar(20),PESO_INI) PESO_INI,  convert(varchar(20), MIN_PESO_INI) MIN_PESO_INI, convert(varchar(20), MAX_PESO_INI) MAX_PESO_INI,convert(varchar(20),ANCHO_INI) ANCHO_INI, convert(varchar(20), MIN_ANCHO_INI) MIN_ANCHO_INI, convert(varchar(20), MAX_ANCHO_INI) MAX_ANCHO_INI, convert(varchar(20),ESPESOR_INI) ESPESOR_INI, VAL_PESO_INI, VAL_ANCHO_INI, VAL_ESPESOR_INI, CUSTOMER_NAME FROM [MTY_PROD_SSM].[dbo].[SSM_INSPECCION_RM] WHERE MOTHER_BOM = '". $FORMERBOM ."'";
											 ...
											 //--- SE CREA EL BOTON DE CONTINUAR ---
												echo '<tr><td></td><td><input type="hidden" name="campo" value="VAL_PESO_INI"><input name="siguiente" id="siguiente" type="submit" class="btn btn-primary" value="Siguiente">&ensp;<input name="continuar" id="continuar" style="display:none;" type="submit" value="Mandar a Rechazo" class="btn btn-primary"onclick="PagRec()"></td></tr></table></form>';
												...
												$(document).ready(function () {
													$('#campovalidar').validate({ 
														...
														rules: {";
															...
															echo "messages: {";
																$consulta = "SELECT MOTHER_BOM,  convert(varchar(20), MIN_PESO_INI) MIN_PESO_INI,  convert(varchar(20), MAX_PESO_INI) MAX_PESO_INI, VAL_PESO_INI  FROM [MTY_PROD_SSM].[dbo].[SSM_INSPECCION_RM] WHERE MOTHER_BOM = '".strtoupper($_GET["bom"])."'";
																	$resultado = odbc_do($conn, $consulta); 
																	while (odbc_fetch_row($resultado)) {
																		echo "".odbc_result($resultado, 1).": '',";
																	}
