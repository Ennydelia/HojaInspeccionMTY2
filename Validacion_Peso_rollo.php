...
<?php
	...
	while (odbc_fetch_row($resultado)) {
		...
		$consulta5 = "EXEC [MTY_PROD_SSM].[dbo].[SP_INSPECCION_T] @BOM_NO = '". strtoupper($_GET["bom"]) ."'";
        odbc_do($conn, $consulta5);
        $consulta6 = "EXEC [MTY_PROD_SSM].[dbo].[SP_INSPECCION_WO_C] @BOM_NO = '". strtoupper($_GET["bom"]) ."'";
		odbc_do($conn, $consulta6);
		//------- SE BUSCA SI YA FUE VALIDADO EL ROLLO MADRE --------//
		$consulta = "select ISNULL(MOTHER_BOM, 'BALANCE') AS MOTHER_BOM FROM [MTY_PROD_SSM].[dbo].[SSM_INSPECCION_RM] WHERE MOTHER_BOM = '". strtoupper($_GET["bom"]) ."' AND WO_NO = '". strtoupper($_GET["wo"]) ."' and FINAL_CHECK is NULL or FINAL_CHECK = 0 AND MOTHER_BOM = '". strtoupper($_GET["bom"]) ."' AND WO_NO = '". strtoupper($_GET["wo"]) ."'";
		$resultado = odbc_do($conn, $consulta);
		$yavalidado = 1;
		while (odbc_fetch_row($resultado)) {
			$FORMERBOM = odbc_result($resultado, 1);
			$yavalidado = 0;
			$consulta = "SELECT count(*) EDO FROM [MTY_PROD_SSM].[dbo].[SSM_INSPECCION_RM] WHERE MOTHER_BOM = '". strtoupper($_GET["bom"]) ."' AND WO_NO = '". strtoupper($_GET["wo"]) ."' and VAL_PESO_INI is NULL ";
			$resultado = odbc_do($conn, $consulta);
			while (odbc_fetch_row($resultado)) {
				if(odbc_result($resultado, 1) <> "0"){
				}
				else{
					header("Location: Validacion_ancho_rollo.php?wo=".$_GET["wo"]."&bom=".$_GET["bom"]);
					die();
				}
			}
		}
...
?>
<!-------- INICIAMOS EL CODIGO HTML/PHP, PARA LOS DATOS A VALIDAR ---->
<!DOCTYPE HTML>
<html lang="es">
<head>
					...
					echo" <script>
						$(document).ready(function () {											
							$('#campovalidar').validate({ 
								...
								echo "messages: {";
									$consulta = "SELECT MOTHER_BOM,  convert(varchar(20), MIN_PESO_INI) MIN_PESO_INI,  convert(varchar(20), MAX_PESO_INI) MAX_PESO_INI, VAL_PESO_INI  FROM [MTY_PROD_SSM].[dbo].[SSM_INSPECCION_RM] WHERE MOTHER_BOM = '".strtoupper($_GET["bom"])."' AND WO_NO = '". strtoupper($_GET["wo"]) ."'";
									$resultado = odbc_do($conn, $consulta); 
									while (odbc_fetch_row($resultado)) {
										echo "".odbc_result($resultado, 1).": '',";
									}
			...
			$.ajax({
				...
				success: function(data) {
					...
					if(res[0]=="Error"){
						...
						window.location.replace("Validacion_Ancho_rollo.php?wo=<?php echo $_GET["wo"]."&bom=".$_GET["bom"]; ?>");
					}
					else{
...
//----------------------FUNCION DE REDIRECCIONAMIENTO A LA PAGINA DE RECHAZOS INTERNOS/CONVIERTE HOJA EN RECHAZO------
	function PagRec(){
		$.confirm({
...
//--------------VALIDACION DE LIBERACION DE PRUEBAS, CAMBIO A CLAVE HG---------
	function Liberar() {
		$.confirm({
