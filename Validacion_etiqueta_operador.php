	...
	while (odbc_fetch_row($resultado)) {
		...
		while (odbc_fetch_row($resultado)) {
			$yavalidado = 0;
			 ...
			 $consulta = "SELECT count(*) EDO FROM [MTY_PROD_SSM].[dbo].[SSM_INSPECCION] WHERE MOTHER_BOM = '".$FORMER_BOM."' AND WO_NO = '". strtoupper($_GET["wo"]) ."' and VAL_ETI_OPERADOR is NULL";// IF HAY NULOS EN LA EVALUACION ANCHO_INICIO
			$resultado = odbc_do($conn, $consulta); 
			while (odbc_fetch_row($resultado)) {
	...
	<div class="row">
		<div class= "col-lg-12 col-md-12 col-sm-12">
			<?php
				...
				while (odbc_fetch_row($resultado)) {
					$FORMER_BOM = odbc_result($resultado, 1);
					$consulta = "SELECT BOM_NO,  convert(varchar(20),MIN_ANCHO) MIN_ANCHO,  convert(varchar(20),MAX_ANCHO) MAX_ANCHO, VAL_ETI_OPERADOR FROM [MTY_PROD_SSM].[dbo].[SSM_INSPECCION] WHERE MOTHER_BOM = '".$FORMER_BOM."' AND WO_NO = '". strtoupper($_GET["wo"]) ."' order by PROD_LINE_NO, BOM_NO";
					$resultado = odbc_do($conn, $consulta); 
          ...
          action: function () {
      	    var name = this.$content.find('.password').val();
						...
						if(name == 'CT101010' || name == 'JJ651510' ||name == 'FP654417' || name == 'AS622461' ||name == "IO734384"||name == 'IO731603' || name == 'IO756514'||name == 'SP916101' || name == 'SP957102'||name == 'SP936703' || name == 'SP991604'||name == 'SP988605'||name == 'SP948506'||name == 'SP928607'||name == 'SP908908'||name == 'SP968009' ||name =='SP934311' ) 
			  		{
							...
							$lugar = "Validacion Etiqueta Operador";
