...
<?php
	...
	while (odbc_fetch_row($resultado)) {
		...
		$consulta = "select MOTHER_BOM from [MTY_PROD_SSM].[dbo].[SSM_INSPECCION_RECHAZO]  WHERE MOTHER_BOM = '". strtoupper($_GET["bom"]) ."' and VAL_CAMBER_FIN is NULL order by PROD_LINE_NO";//OBTIENE LOS FORMERS BOMS DE ESE WO
		$resultado = odbc_do($conn, $consulta);	
		while (odbc_fetch_row($resultado)) {
			$FORMER_BOM = odbc_result($resultado, 1);
			$consulta = "SELECT count(*) EDO FROM [MTY_PROD_SSM].[dbo].[SSM_INSPECCION_RECHAZO] WHERE MOTHER_BOM = '".$FORMER_BOM."' and VAL_CAMBER_FIN is NULL";// IF HAY NULOS EN LA EVALUACION ANCHO_INICIO
			$resultado = odbc_do($conn, $consulta);	
			while (odbc_fetch_row($resultado)) {
				if(odbc_result($resultado, 1) <> "0"){
					...
					header("Location: Rechazo_carlite_fin.php?wo=".$_GET["wo"]."&bom=".$_GET["bom"]);
					die();
				}
			}
		}
	...
	<div class="row">
		<div class= "col-lg-12 col-md-12 col-sm-12">
			<?php
				...
				while (odbc_fetch_row($resultado)) {
					$FORMER_BOM = odbc_result($resultado, 1);
					$consulta = "SELECT BOM_NO, convert(varchar(20), 0) C1,  convert(varchar(20), CAMBER) C2, VAL_CAMBER_FIN FROM [MTY_PROD_SSM].[dbo].[SSM_INSPECCION_RECHAZO] WHERE MOTHER_BOM = '".$FORMER_BOM."'  order by PROD_LINE_NO, BOM_NO";
					//ND (((MIN_ANCHO + MAX_ANCHO) / 2) 
					$resultado = odbc_do($conn, $consulta);
					echo "<center><h4>VALIDACION FINAL CAMBER (RECHAZO)</h4></center>";	
			...
			//AQUI SE CAMBIA EL CAMPO A INSERTAR -------------------------------V
					echo '<tr><td></td><td><input type="hidden" name="campo" value="VAL_CAMBER_FIN"><input name="siguiente" id="siguiente" type="submit" class="btn btn-primary" value="Siguiente">&ensp;<input name="continuar" id="continuar" style="display:none;" type="button" value="Mandar a Rechazo" class="btn btn-danger"onclick="PagRec()"></td></tr></table></form>';
				}
			?>
		</div>
	</div>
				...
				$.ajax({
					...
					success: function(data) {
						...
						if(res[0]=="Error"){
							...
							window.location.replace("Rechazo_carlite_fin.php?wo=<?php echo $_GET["wo"]."&bom=".$_GET["bom"]; ?>");
						}
						else{
