				 ...
				 <?php
					...
					while (odbc_fetch_row($resultado)) {
						...
						echo "<h3>". strtoupper($_GET["wo"]) ." o ". strtoupper($_GET["bom"]) . "no existen.</h3>";
						 ...
						 $consulta = "select top 1 MACHINE_CD existe_wo from openquery(hgdb,'select MACHINE_CD from WK04_WO_HEADER where company_cd = ''MTY'' and WO_NO = ''". strtoupper($_GET["wo"]) ."'' ')";
							...
							$consulta = "select MOTHER_BOM from [MTY_PROD_SSM].[dbo].[SSM_INSPECCION_RECHAZO]  WHERE MOTHER_BOM = '". strtoupper($_GET["bom"]) ."' order by PROD_LINE_NO";//OBTIENE LOS FORMERS BOMS DE ESE WO
								...
								$consulta = "SELECT count(*) EDO FROM [MTY_PROD_SSM].[dbo].[SSM_INSPECCION_RECHAZO] WHERE MOTHER_BOM = '".$FORMER_BOM."' and  VAL_INI_REBABA_OP IS NULL";// IF HAY NULOS EN LA EVALUACION ANCHO_INICIO
								$resultado = odbc_do($conn, $consulta); 
								while (odbc_fetch_row($resultado)) {
                      ...
                      while (odbc_fetch_row($resultado)) {
                        ...
                        echo '<td><input style="width:100px;" autocomplete="off" lang="es" type="text"  id="'.odbc_result($resultado, 1).'" name="'.odbc_result($resultado, 1).'" value="'.odbc_result($resultado, 5).'"></td>';
                        ...
                        $count++;
									 ...
									 echo '<tr><td></td><td><input type="hidden" name="campo" value="VAL_INI_REBABA_OP"><input id="Siguiente" type="submit" class="btn btn-primary" value="Siguiente">&ensp;<input id="continuar" style="display:none;" type="submit" value="Mandar a Rechazo" class="btn btn-primary"onclick="PagRec()"></td></tr></table></form>';

										
								}
									else{
										//REDIRIGE A LA SIGUIENTE EVALUCION (ONDULACION INICIAL)
