...
<?php
	...
	while (odbc_fetch_row($resultado)) {
		...
		$consulta = "select MOTHER_BOM from [MTY_PROD_SSM].[dbo].[SSM_INSPECCION_RECHAZO]  WHERE MOTHER_BOM = '". strtoupper($_GET["bom"]) ."' order by PROD_LINE_NO";//FORMERS BOM
		$resultado = odbc_do($conn, $consulta);	
		while (odbc_fetch_row($resultado)) {
			$FORMER_BOM = odbc_result($resultado, 1);
			$consulta = "SELECT count(*) EDO FROM [MTY_PROD_SSM].[dbo].[SSM_INSPECCION_RECHAZO] WHERE MOTHER_BOM = '".$FORMER_BOM."' and VAL_CARLITE_INICIO is NULL";// IF HAY NULOS EN LA EVALUACION ANCHO_INICIO
			$resultado = odbc_do($conn, $consulta);
			$yavalidado = 1;
			while (odbc_fetch_row($resultado)) {
				$yavalidado = 0;
		...
		<div class= "col-lg-12 col-md-12 col-sm-12">
			<?php
				...
				while (odbc_fetch_row($resultado)) {
					...
					$resultado = odbc_do($conn, $consulta);
					echo "<center><h4>VALIDACION CARLITE INICIO (RECHAZO) </h4></center>";	
				...
				$.ajax({
					...
					success: function(data) {
						...
						if(res[0]=="Error"){
							...
							toastr.success(res[1], 'Datos insertados correctamente', {timeOut: 2500, positionClass: "toast-top-center"});
