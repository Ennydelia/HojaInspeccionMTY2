						...
						echo "<h3>". strtoupper($_GET["wo"]) . " o " . strtoupper($_GET["bom"]) ." no existen.</h3>";
							...
							while (odbc_fetch_row($resultado)) {
								...
								$resultado4 = odbc_do($conn, $consulta4);
												...
												$consulta = "select MOTHER_BOM FROM [MTY_PROD_SSM].[dbo].[SSM_INSPECCION_RM_RECHAZO] WHERE MOTHER_BOM = '". strtoupper($_GET["bom"]) ."'";//OBTIENE EL ROLLO MADRE QUE NO HALLA VALIDADO EL ESPESOR
												...
												while (odbc_fetch_row($resultado)) {
													...
													$consulta = "SELECT count(*) EDO FROM [MTY_PROD_SSM].[dbo].[SSM_INSPECCION_RM_RECHAZO] WHERE MOTHER_BOM = '". strtoupper($_GET["bom"]) ."' AND VAL_ESPESOR_INI is NULL";
													$resultado = odbc_do($conn, $consulta); 
												 while (odbc_fetch_row($resultado)) {
														if(odbc_result($resultado, 1) <> "0"){
															...
															echo '<tr><td></td><td><input type="hidden" name="campo" value="VAL_ESPESOR_INI"><input id="Siguiente" type="submit" class="btn btn-primary" value="Siguiente">&ensp;<input id="continuar" style="display:none;" type="submit" value="Mandar a Rechazo" class="btn btn-primary"onclick="PagRec()"></td></tr></table></form>';
															//--- INICIAMOS VALIDACION DE LOS 3 CAMPOS ---
																																		}
																				else{
																					//MOSTRAR LA SIGUIENTE EVALUACION (ANCHOS INICIO)
																					header("Location: rechazo_ancho_inicio.php?wo=".$_GET["wo"]."&bom=".$_GET["bom"]);
																					die();
																				}
																		}
																	}
										...
										$.ajax({
												...
												success: function(data) {
															...
															if(res[0]=="Error"){
																...
																window.location.replace("rechazo_ancho_inicio.php?wo=<?php echo $_GET["wo"]."&bom=".$_GET["bom"]; ?>");
															}
															else{
