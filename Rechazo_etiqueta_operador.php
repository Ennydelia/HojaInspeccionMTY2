						...
						echo "<h3>". strtoupper($_GET["wo"]) ." o ". strtoupper($_GET["bom"]) . "no existen.</h3>";
						 ...
						 $consulta = "select top 1 MACHINE_CD existe_wo from openquery(hgdb,'select MACHINE_CD from WK04_WO_HEADER where company_cd = ''MTY'' and WO_NO = ''". strtoupper($_GET["wo"]) ."'' ')";
						  ...
						  $maquina = odbc_result($resultado, 1);//ESTA ES LA MAQUINA DONDE SE CORTA SLITTER SE MIDE AL PRINCIPIO Y AL FINAL          	
							$consulta = "select MOTHER_BOM from [MTY_PROD_SSM].[dbo].[SSM_INSPECCION_RECHAZO]  WHERE MOTHER_BOM = '". strtoupper($_GET["bom"]) ."' and VAL_ETI_OPERADOR is NULL AND MOTHER_BOM = '". strtoupper($_GET["bom"]) ."' order by PROD_LINE_NO";//OBTIENE LOS FORMERS BOMS DE ESE WO
							  $resultado = odbc_do($conn, $consulta);  
							  $yavalidado = 1;
							  while (odbc_fetch_row($resultado)) {
								$yavalidado = 0;
								$FORMER_BOM = odbc_result($resultado, 1);
								$consulta = "SELECT count(*) EDO FROM [MTY_PROD_SSM].[dbo].[SSM_INSPECCION_RECHAZO] WHERE MOTHER_BOM = '".$FORMER_BOM."' and VAL_ETI_OPERADOR is NULL";// IF HAY NULOS EN LA EVALUACION ANCHO_INICIO
								$resultado = odbc_do($conn, $consulta); 
								while (odbc_fetch_row($resultado)) {
								  if(odbc_result($resultado, 1) <> "0"){//SI HAY NULOS MUESTRA LOS CAMPOS PARA LLENAR VALORES
									  $consulta = "SELECT BOM_NO,  convert(varchar(20),MIN_ANCHO) MIN_ANCHO,  convert(varchar(20),MAX_ANCHO) MAX_ANCHO, VAL_ETI_OPERADOR FROM [MTY_PROD_SSM].[dbo].[SSM_INSPECCION_RECHAZO] WHERE MOTHER_BOM = '".$FORMER_BOM."' order by PROD_LINE_NO, BOM_NO";
									  $resultado = odbc_do($conn, $consulta); 
									  echo "<center><h4>ETIQUETA OPERADOR (RECHAZO)</h4></center>";
									 ...
									 echo '<tr><td></td><td><input type="hidden" name="campo" value="VAL_ETI_OPERADOR"><input id="Siguiente" type="submit" class="btn btn-primary" value="Siguiente">&ensp;<input id="continuar" style="display:none;" type="submit" value="Mandar a Rechazo" class="btn btn-primary"onclick="PagRec()"></td></tr></table></form>';
						 ...
						 If ($yavalidado == 1){
									  ...
									  header("Location: Rechazo_etiqueta_supervisor.php?wo=".$_GET["wo"]."&bom=".$_GET["bom"]);
									  die();
						  }
					  }
				   }
					...
					$.ajax({
						...
						success: function(data) {
							  ...
							  if(res[0]=="Error"){
								...
								window.location.replace("Rechazo_etiqueta_supervisor.php?wo=<?php echo $_GET["wo"]."&bom=".$_GET["bom"]; ?>");
							  }
							  else{
