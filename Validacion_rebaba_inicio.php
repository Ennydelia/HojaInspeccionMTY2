...
<?php
	...
	while (odbc_fetch_row($resultado)) {
		...
		while (odbc_fetch_row($resultado)) {
			...
			$consulta = "SELECT count(*) EDO FROM [MTY_PROD_SSM].[dbo].[SSM_INSPECCION] WHERE MOTHER_BOM = '".$FORMER_BOM."' AND WO_NO = '". strtoupper($_GET["wo"]) ."' and VAL_INI_REBABA_MOTOR is NULL and VAL_INI_REBABA_OP IS NULL";// IF HAY NULOS EN LA EVALUACION ANCHO_INICIO
			$resultado = odbc_do($conn, $consulta); 
			while (odbc_fetch_row($resultado)) {
		...
		<div class= "col-lg-12 col-md-12 col-sm-12">
			<?php
				...
				while (odbc_fetch_row($resultado)) {
					...
					$consulta = "SELECT BOM_NO,  convert(varchar(20),0) R1,  convert(varchar(20),REBABA) R2, VAL_INI_REBABA_MOTOR, VAL_INI_REBABA_OP, PROD_LINE_NO FROM [MTY_PROD_SSM].[dbo].[SSM_INSPECCION] WHERE MOTHER_BOM = '".$FORMERBOM."' AND WO_NO = '". strtoupper($_GET["wo"]) ."' order by PROD_LINE_NO, BOM_NO";
					$resultado = odbc_do($conn, $consulta);	
					...
					echo '<table id="tabla-valor" class="table" style="width:100%"><tr><th colspan="3">ROLLO MADRE: '.$FORMERBOM.'</th></tr><tr><th>BOM</th><th>MOTOR </th><th>OPERADOR </th></tr>';
					$count = 1;
					while (odbc_fetch_row($resultado)) {
					...
					<input type="hidden" name="bomm" id="bomm" value= '. strtoupper($_GET["bom"]).'><input name="siguiente" id="siguiente" type="submit" class="btn btn-primary" value="Siguiente">&ensp;<input name="continuar" id="continuar" style="display:none;" type="button" value="Mandar a Rechazo" class="btn btn-danger"onclick="PagRec()"></td><td></td></tr></table></form>';
					...
					echo" <script>
						$(document).ready(function () {
							$('#campovalidar').validate({ 
								...
								rules: {";
									$consulta = "SELECT BOM_NO, convert(varchar(20),0) R1,  convert(varchar(20),REBABA) R2, VAL_INI_REBABA_MOTOR, VAL_INI_REBABA_OP, PROD_LINE_NO FROM [MTY_PROD_SSM].[dbo].[SSM_INSPECCION] WHERE MOTHER_BOM = '".$FORMER_BOM."' AND WO_NO = '". strtoupper($_GET["wo"]) ."' order by PROD_LINE_NO, BOM_NO";
									$resultado = odbc_do($conn, $consulta); 
									$count = 1;
									while (odbc_fetch_row($resultado)) {
...
<script>
	...
	//Desactiva la tecla enter al tener el visible el boton de rechazos 
