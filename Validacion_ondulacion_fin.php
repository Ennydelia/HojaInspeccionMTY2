...
<?php
	...
	while (odbc_fetch_row($resultado)) {
		...
		while (odbc_fetch_row($resultado)) {
			...
			$consulta = "SELECT count(*) EDO FROM [MTY_PROD_SSM].[dbo].[SSM_INSPECCION] WHERE MOTHER_BOM = '".$FORMER_BOM."' AND WO_NO = '". strtoupper($_GET["wo"]) ."' and VAL_ONDULACION_FIN is NULL";// IF HAY NULOS EN LA EVALUACION ANCHO_INICIO
			$resultado = odbc_do($conn, $consulta);	
			while (odbc_fetch_row($resultado)) {
				if(odbc_result($resultado, 1) <> "0"){
					...
					header("Location: Validacion_num_ondas_fin.php?wo=".$_GET["wo"]."&bom=".$_GET["bom"]);
					die();
				}
			}
		}
	...
	<div class="row">
		<div class= "col-lg-12 col-md-12 col-sm-12">
			<?php
				...
				while (odbc_fetch_row($resultado)) {
					$FORMER_BOM = odbc_result($resultado, 1);
					$consulta = "SELECT BOM_NO, convert(varchar(20), 0) C1,  convert(varchar(20), ONDULACIONES) C2, VAL_ONDULACION_FIN FROM [MTY_PROD_SSM].[dbo].[SSM_INSPECCION] WHERE MOTHER_BOM = '".$FORMER_BOM."' AND WO_NO = '". strtoupper($_GET["wo"]) ."' order by PROD_LINE_NO, BOM_NO";
					// AND (((MIN_ANCHO + MAX_ANCHO) / 2) <= 200 OR CLIENTE LIKE '%EUROTRANCIATURA%' OR CLIENTE LIKE '%SIEMENS%' )
					$resultado = odbc_do($conn, $consulta);
					echo "<center><h4>VALIDACION FINAL ONDULACION</h4></center>";	
            ...
            success: function(data) {
              ...
              var res = str.split(",");
															...
															if(res[0]=="Error"){
																...
																window.location.replace("Validacion_num_ondas_fin.php?wo=<?php echo $_GET["wo"]."&bom=".$_GET["bom"]; ?>");
          ...
          action: function () {
      	    var name = this.$content.find('.password').val();
						...
						if(name == 'CT101010' || name == 'JJ651510' ||name == 'FP654417' || name == 'AS622461' ||name == "IO734384"||name == 'IO731603' || name == 'IO756514'||name == 'SP916101' || name == 'SP957102'||name == 'SP936703' || name == 'SP991604'||name == 'SP988605'||name == 'SP948506'||name == 'SP928607'||name == 'SP908908'||name == 'SP985361' ||name =='SP934311' ) 
			  		{
							...
							$lugar = "Validacion Ondulacion Fin";
