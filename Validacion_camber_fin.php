...
<?php
	...
	while (odbc_fetch_row($resultado)) {
		...
		while (odbc_fetch_row($resultado)) {
			...
			$consulta = "SELECT count(*) EDO FROM [MTY_PROD_SSM].[dbo].[SSM_INSPECCION] WHERE MOTHER_BOM = '".$FORMER_BOM."'  AND WO_NO = '". strtoupper($_GET["wo"]) ."' and VAL_CAMBER_FIN is NULL";// IF HAY NULOS EN LA EVALUACION ANCHO_INICIO
			$resultado = odbc_do($conn, $consulta);	
			while (odbc_fetch_row($resultado)) {
	...
	<div class="row">
		<div class= "col-lg-12 col-md-12 col-sm-12">
			<?php
				...
				while (odbc_fetch_row($resultado)) {
					$FORMER_BOM = odbc_result($resultado, 1);
					$consulta = "SELECT BOM_NO, convert(varchar(20), 0) C1,  convert(varchar(20), CAMBER) C2, VAL_CAMBER_FIN FROM [MTY_PROD_SSM].[dbo].[SSM_INSPECCION] WHERE MOTHER_BOM = '".$FORMER_BOM."' AND WO_NO = '". strtoupper($_GET["wo"]) ."' order by PROD_LINE_NO, BOM_NO";
					$resultado = odbc_do($conn, $consulta);
				...
				$.ajax({
					...
					success: function(data) {
						...
						if(res[0]=="Error"){
							...
							window.location.replace("Validacion_carlite_fin.php?wo=<?php echo $_GET["wo"]."&bom=".$_GET["bom"]; ?>");
						}
						else{
...
//------------------FUNCION QUE REDIRIGE A LA PAGINA DE RECHAZOS INTERNOS-------------------------------------
	function PagRec(){
		$.confirm({
