...
<?php
	...
	while (odbc_fetch_row($resultado)) {
		...
		$consulta = "select MOTHER_BOM FROM [MTY_PROD_SSM].[dbo].[SSM_INSPECCION_RM] WHERE MOTHER_BOM = '". strtoupper($_GET["bom"]) ."' AND WO_NO = '". strtoupper($_GET["wo"]) ."' and FINAL_CHECK is NULL or FINAL_CHECK = 0 AND MOTHER_BOM = '". strtoupper($_GET["bom"]) ."' AND WO_NO = '". strtoupper($_GET["wo"]) ."'";//OBTIENE EL ROLLO MADRE QUE NO HALLA VALIDADO EL ESPESOR
		$resultado = odbc_do($conn, $consulta); 
		$yavalidado = 1;
		while (odbc_fetch_row($resultado)) {
			$yavalidado = 0;
			//--- REVISA QUE EXISTAN CAMPOS NULOS EN LA CONSULTA
			$consulta = "SELECT count(*) EDO FROM [MTY_PROD_SSM].[dbo].[SSM_INSPECCION_RM] WHERE MOTHER_BOM = '". strtoupper($_GET["bom"]) ."' AND WO_NO = '". strtoupper($_GET["wo"]) ."' AND VAL_ESPESOR_INI is NULL";
			$resultado = odbc_do($conn, $consulta); 
		   while (odbc_fetch_row($resultado)) {
		...
		<div class= "col-lg-12 col-md-12 col-sm-12">
			<?php
				...
				while (odbc_fetch_row($resultado)) {
					...
					echo '<tr><td>'.odbc_result($resultado, 1).'</td><td><input style="width:100px;" autocomplete="off" autofocus="on" lang="es" type="number" id="'.odbc_result($resultado, 1).'" name="'.odbc_result($resultado, 1).'" value="'.odbc_result($resultado, 13).'"></td><td></td><td></td></tr>';
					$count++;
					}
					...
					echo" <script>
						$(document).ready(function () {
							...
							echo "messages: {";
								$consulta = "SELECT MOTHER_BOM,  convert(varchar(20), MIN_ESPESOR_INI) MIN_ESPESOR_INI,  convert(varchar(20), MAX_ESPESOR_INI) MAX_ESPESOR_INI, VAL_ESPESOR_INI  FROM [MTY_PROD_SSM].[dbo].[SSM_INSPECCION_RM] WHERE MOTHER_BOM = '".strtoupper($_GET["bom"])."' AND WO_NO = '". strtoupper($_GET["wo"]) ."'";
								$resultado = odbc_do($conn, $consulta); 
								while (odbc_fetch_row($resultado)) {
									echo "".odbc_result($resultado, 1).": '',";
								}
