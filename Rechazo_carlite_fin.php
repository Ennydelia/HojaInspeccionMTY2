              ...
              while (odbc_fetch_row($resultado)) {
                ...
                while (odbc_fetch_row($resultado)) {
                  if(odbc_result($resultado, 1) <> "0"){//SI HAY NULOS MUESTRA LOS CAMPOS PARA LLENAR VALORES
										...
										$count = 1;
                  while (odbc_fetch_row($resultado)) {
                    echo '<tr><td><abbr title="'.odbc_result($resultado, 2).'" rel="tooltip">'.odbc_result($resultado, 1).'</abbr></td><td><input style="width:100px;" autocomplete="off" autofocus="on" lang="es" type="text" id="'.odbc_result($resultado, 1).'" name="'.odbc_result($resultado, 1).'" value="'.odbc_result($resultado, 3).'"></td><td><input style="display:none;"  id="'.odbc_result($resultado, 1).'" name="'.odbc_result($resultado, 1).'" value="'.odbc_result($resultado, 2).'"></td></tr>';
                    $count++;
                  }
                  //AQUI SE CAMBIA EL CAMPO A INSERTAR -------------------------------
                  echo '<tr><td></td><td><input type="hidden" name="campo" value="VAL_CARLITE_FIN"><input id="Siguiente" type="submit" class="btn btn-primary" value="Siguiente">&ensp;<input id="continuar" style="display:none;" type="submit" value="Mandar a Rechazo" class="btn btn-primary"onclick="PagRec()"></td></tr></table></form>';
            ...
            $.ajax({
              ...
              success: function(data) {
                ...
                if(res[0]=="Error"){
                  ...
                  toastr.success(res[1], 'Informacion del Rechazo Guardado Correctamente', {timeOut: 2500, positionClass: "toast-top-center"});
