...
<?php
	...
	while (odbc_fetch_row($resultado)) {
		...
		while (odbc_fetch_row($resultado)) {
			...
			$consulta = "SELECT count(*) EDO FROM [MTY_PROD_SSM].[dbo].[SSM_INSPECCION] WHERE MOTHER_BOM = '".$FORMER_BOM."'  AND WO_NO = '". strtoupper($_GET["wo"]) ."' and VAL_NUM_ONDAS_FIN is NULL";// IF HAY NULOS EN LA EVALUACION ANCHO_INICIO
			$resultado = odbc_do($conn, $consulta);	
			while (odbc_fetch_row($resultado)) {
	...
	<div class="row">
		<div class= "col-lg-12 col-md-12 col-sm-12">
			<?php
				...
				while (odbc_fetch_row($resultado)) {
					$FORMER_BOM = odbc_result($resultado, 1);
					$consulta = "SELECT BOM_NO, convert(varchar(20), 0) C1,  convert(varchar(20), NUM_ONDAS) C2, VAL_NUM_ONDAS_FIN FROM [MTY_PROD_SSM].[dbo].[SSM_INSPECCION] WHERE MOTHER_BOM = '".$FORMER_BOM."' AND WO_NO = '". strtoupper($_GET["wo"]) ."' order by PROD_LINE_NO, BOM_NO";															
					$resultado = odbc_do($conn, $consulta);
					...
					echo" <script>
						$(document).ready(function () {
							$('#campovalidar').validate({ 
								...
								echo "messages: {";
									$consulta = "SELECT BOM_NO, convert(varchar(20), 0) C1,  convert(varchar(20), 5) C2, VAL_NUM_ONDAS_FIN FROM [MTY_PROD_SSM].[dbo].[SSM_INSPECCION] WHERE MOTHER_BOM = '".$FORMER_BOM."' AND WO_NO = '". strtoupper($_GET["wo"]) ."' order by PROD_LINE_NO, BOM_NO";
									$resultado = odbc_do($conn, $consulta);	
									while (odbc_fetch_row($resultado)) {
										echo "".odbc_result($resultado, 1).": '',";
									}
            ...
            success: function(data) {
              ...
              var res = str.split(",");
															if(res[0]=="Error"){
																...
																window.location.replace("Validacion_camber_fin.php?wo=<?php echo $_GET["wo"]."&bom=".$_GET["bom"]; ?>");
