...
<?php
	...
	while (odbc_fetch_row($resultado)) {
		...
		while (odbc_fetch_row($resultado)) {
			...
			$consulta = "SELECT count(*) EDO FROM [MTY_PROD_SSM].[dbo].[SSM_INSPECCION_RECHAZO] WHERE MOTHER_BOM = '".$FORMER_BOM."' and VAL_INI_REBABA_MOTOR is NULL and VAL_INI_REBABA_OP IS NULL";// IF HAY NULOS EN LA EVALUACION ANCHO_INICIO
			$resultado = odbc_do($conn, $consulta); 
			while (odbc_fetch_row($resultado)) {
				if(odbc_result($resultado, 1) <> "0"){
					...
					header("Location: Rechazo_ondulacion_inicio.php?wo=".$_GET["wo"]."&bom=".$_GET["bom"]);
					die();
				}
			}
		}
					...
					success: function(data) {
						...
						var res = str.split(",");
							if(res[0]=="Error"){
								...
								window.location.replace("Rechazo_ondulacion_inicio.php?wo=<?php echo $_GET["wo"]."&bom=".$_GET["bom"]; ?>");
							}
							else{
