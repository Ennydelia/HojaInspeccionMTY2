		...
		<div class= "col-lg-12 col-md-12 col-sm-12">
			<?php
				...
				while (odbc_fetch_row($resultado)) {
					...
					$consulta = "select MOTHER_BOM FROM [MTY_PROD_SSM].[dbo].[SSM_INSPECCION_RM] WHERE MOTHER_BOM = '". strtoupper($_GET["bom"]) ."' and FINAL_CHECK is NULL or FINAL_CHECK = 0 AND MOTHER_BOM = '". strtoupper($_GET["bom"]) ."'";
					$resultado = odbc_do($conn, $consulta); 
					$yavalidado = 1;
					while (odbc_fetch_row($resultado)) {
						$yavalidado = 0;
						//--- REVISA QUE EXISTAN CAMPOS NULOS EN LA CONSULTA
						$consulta = "SELECT count(*) EDO FROM [MTY_PROD_SSM].[dbo].[SSM_INSPECCION_RM] WHERE MOTHER_BOM = '". strtoupper($_GET["bom"]) ."'  AND VAL_ANCHO_INI IS NULL ";
						$resultado = odbc_do($conn, $consulta); 
						while (odbc_fetch_row($resultado)) {
								...
								echo" <script>
									$(document).ready(function () {
										$('#campovalidar').validate({ 
											...
											echo "messages: {";
												$consulta = "SELECT MOTHER_BOM,  convert(varchar(20), MIN_ANCHO_INI) MIN_ANCHO_INI,  convert(varchar(20), 	MAX_ANCHO_INI) MAX_ANCHO_INI, VAL_ANCHO_INI  FROM [MTY_PROD_SSM].[dbo].[SSM_INSPECCION_RM] WHERE MOTHER_BOM = '".strtoupper($_GET["bom"])."'";
												$resultado = odbc_do($conn, $consulta); 
												while (odbc_fetch_row($resultado)) {
													echo "".odbc_result($resultado, 1).": '',";
												}
